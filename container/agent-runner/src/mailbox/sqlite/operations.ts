...
export function sqliteGetMessageIdBySeq(sequence: number): string | null {
  ...
  const inboundRow = inbound.prepare('SELECT id FROM messages_in WHERE seq = ?').get(sequence) as
    | { id: string }
    | undefined;
  // messages_in.id is the platform message id namespaced with a trailing
  // ":<agentGroupId>" by the host's messageIdForAgent() (src/router.ts) so the
  // PK stays unique when one inbound message fans out to several agent-group
  // sessions. Callers here want the platform id: edit_message / add_reaction
  // send it straight back to the channel, and the suffix makes it an id the
  // platform has never seen (Matrix rejects it outright with "Can't send
  // relation to unknown event"; other platforms may silently misfire).
  if (inboundRow) {
    const { agentGroupId } = loadConfig();
    const suffix = `:${agentGroupId}`;
    return agentGroupId && inboundRow.id.endsWith(suffix) ? inboundRow.id.slice(0, -suffix.length) : inboundRow.id;
  }
